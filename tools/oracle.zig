...
//!   * Patterns with an empty-bodied loop are skipped: whether such an
//!     iteration's captures survive is a documented difference between
//!     backtracking and automaton engines, and zregex sides with the
//!     automaton (see src/fuzz.zig).
...
pub fn main(init: std.process.Init) !void {
    ...
    for (0..cases) |_| {
        ...
        if (bad) {
            disagreements += 1;
            std.debug.print("DISAGREE pattern={s}\n  haystack={f}\n", .{
    ...
    std.debug.print("compared {d}, skipped {d}, disagreements {d}\n", .{ compared, skipped, disagreements });
